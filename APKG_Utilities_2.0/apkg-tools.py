...
class Apkg:
    ...
    def create(self, folder, dest_dir=None):
        ...
        apk_file = dest_dir + '/' + app_info['general']['package'] + '_' + app_info['general']['version'] + \
            '_' + app_info['general']['architecture'] + \
        ...
        # archive apk file
        with Chdir(tmp_dir):
            self.__zip_archive(apk_file, [self.apk_file_contents['version'],
                                          self.apk_file_contents['control'], self.apk_file_contents['data']])
    ...
    def extract(self, package, dest_dir=None):
        ...
        self.__zip_extract(
            apk_file, self.apk_file_contents['version'], tmp_contents_dir)
        self.__zip_extract(
            apk_file, self.apk_file_contents['control'], tmp_contents_dir)
        self.__zip_extract(
            apk_file, self.apk_file_contents['data'], tmp_contents_dir)

        # unpack package phase 2
        tmp_control_dir = tmp_dir + '/' + self.apk_special_folders['control']
        os.mkdir(tmp_control_dir)

        self.__tar_extract(tmp_contents_dir + '/' +
                           self.apk_file_contents['control'], tmp_control_dir)
        self.__tar_extract(tmp_contents_dir + '/' +
                           self.apk_file_contents['data'], tmp_dir)

        # get apkg version
        apkg_version = self.__get_apkg_version(
            tmp_contents_dir + '/' + self.apk_file_contents['version'])
        ...
        if apkg_version == '1.0':
            app_dir = dest_dir + '/' + \
                apk_info['app']['name'] + '_' + apk_info['app']['version'] + \
                '_' + apk_info['app']['architecture']
        elif apkg_version == '2.0':
            app_dir = dest_dir + '/' + apk_info['general']['name'] + '_' + \
                apk_info['general']['version'] + '_' + \
    ...
    def convert(self, package):
        ...
        description_file = control_dir + '/' + \
            self.apk_control_files['description']

        # get old format app information
        app_old_info = self.__get_app_info(
            control_dir, self.apk_format['version'])
        ...
        app_new_info['general']['developer'] = app_old_info['app']['website'] if (
            developer is None) else developer
...
if __name__ == "__main__":
    ...
    parser_create = subparsers.add_parser(
        'create', help='create package from folder')
    parser_create.add_argument(
        'folder', help='select a package layout folder to pack')
    parser_create.add_argument(
        '--destination', help='move apk to destination folder')
    parser_create.set_defaults(command='create')

    # create the parser for the "extract" commad
    parser_extract = subparsers.add_parser(
        'extract', help='extract package to folder')
    parser_extract.add_argument('package', help='select a package to extract')
    parser_extract.add_argument(
        '--destination', help='extract apk to destination folder')
